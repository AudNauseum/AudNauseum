...
class Ui_Dialog(object):
    def setupUi(self, Dialog):
        ...
        # Custom limits for volume dial
        self.dial_volume.setMaximum(0)
        self.dial_volume.setMaximum(100)
        self.dial_volume.setValue(30)
...
"    }")
        ...
        self.pushButton_record.clicked.connect(lambda:self.whichbtn(self.pushButton_record))
        self.pushButton_play.clicked.connect(lambda:self.whichbtn(self.pushButton_play))
        self.pushButton_stop.clicked.connect(lambda:self.whichbtn(self.pushButton_stop))
        self.pushButton_shift.clicked.connect(lambda:self.whichbtn(self.pushButton_shift))

        # TRACK CONTROL - Add listeners to each button in track controls group
        self.pushButton_add_track.clicked.connect(lambda:self.whichbtn(self.pushButton_add_track))
        self.pushButton_rem_track.clicked.connect(lambda:self.whichbtn(self.pushButton_rem_track))
        self.pushButton_new_track.clicked.connect(lambda:self.whichbtn(self.pushButton_new_track))
        self.pushButton_solo_track.clicked.connect(lambda:self.whichbtn(self.pushButton_solo_track))

        # EFFECTS (FX) - Add listeners to each button in effects control group
        self.pushButton_pan_beats.clicked.connect(lambda:self.whichbtn(self.pushButton_pan_beats))
        self.pushButton_pitch.clicked.connect(lambda:self.whichbtn(self.pushButton_pitch))
        self.pushButton_slip.clicked.connect(lambda:self.whichbtn(self.pushButton_slip))
        self.pushButton_reverse.clicked.connect(lambda:self.whichbtn(self.pushButton_reverse))

        # METRONOME - Add listener to toggle metronome on or off
        self.pushButton_metro_on_off.clicked.connect(lambda:self.whichbtn(self.pushButton_metro_on_off))

        # VOLUME - Add listener for volume control
        self.dial_volume.valueChanged.connect(self.dial_value)

        # TRACK SELECT - Add listener to get value of track select
        self.spinBox_context.valueChanged.connect(self.spinbox_value)


        self.lcdNumber.display(8888)
        
        # This function allows setting up a depressed and non-depressed button feature
        # self.pushButton_solo_track.clicked.connect(self.btnstate)
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label_21.setText(_translate("Dialog", "TRACK SELECT"))
        self.label_13.setText(_translate("Dialog", "SHIFT"))

               #     def btnstate(self):
        #       if self.pushButton_solo_track.isChecked():
        #          print("solo button pressed")
        #       else:
        #          print("solo button released")

    def whichbtn(self, b):
        print("clicked button is "+b.objectName())

    def dial_value(self):
        getValue = self.dial_volume.value()
        print("volume value is", str(getValue))

    def spinbox_value(self):
        getValue = self.spinBox_context.value()
